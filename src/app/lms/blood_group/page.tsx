...
const Blood_groupSettings: React.FC = () => {
  // Modal
  ...
  // Table
  const [blood_groups, setBlood_groups] = useState<Blood_group[]>([]);
  ...
  // Fetch Blood_groups
  const fetchBlood_groups = async () => {
    const res = await axios.get(`/api/settings/blood_group`);
    if (res.data.ok) setBlood_groups(res.data.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/blood_group/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/blood_group`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/blood_group/${id}`);
    fetchBlood_groups();
  };

  // Filter + Pagination
  const filtered = blood_groups.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Blood groups</h5>
      ...
      <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
        <div className="trezo-card-header mb-[20px] md:mb-[25px] sm:flex items-center justify-between">
          ...
          <form className="relative sm:w-[265px]">
            ...
            <input
              type="text"
              placeholder="Search Blood_group..."
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <DialogPanel className="bg-white dark:bg-[#0c1427] p-6 rounded-md w-full max-w-md">
            <h3 className="text-lg font-medium mb-4">
              {editId ? "Edit blood group" : "Add blood group"}
            </h3>
...
export default Blood_groupSettings;
