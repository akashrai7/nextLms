...
const SectionSettings: React.FC = () => {
  ...
  const fetchSections = async () => {
    const res = await axios.get(`/api/settings/section`);
    if (res.data.ok) setSections(res.data.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/section/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/section`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/section/${id}`);
    fetchSections();
  };

  // Filter + Pagination
  const filtered = sections.filter((g) =>
