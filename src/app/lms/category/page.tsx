...
const CategorySettings: React.FC = () => {
  ...
  const fetchCategorys = async () => {
    const res = await axios.get(`/api/settings/category`);
    if (res.data.ok) setCategorys(res.data.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/category/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/category`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/category/${id}`);
    fetchCategorys();
  };

  // Filter + Pagination
  const filtered = categorys.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Categorys</h5>
