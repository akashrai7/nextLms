...
const StreamSettings: React.FC = () => {
  ...
  const fetchStreams = async () => {
    const res = await axios.get(`/api/settings/stream`);
    if (res.data.ok) setStreams(res.data.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/stream/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/stream`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/stream/${id}`);
    fetchStreams();
  };

  // Filter + Pagination
  const filtered = streams.filter((g) =>
