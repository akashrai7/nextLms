...
interface Course_level {
  _id: string;
  name: string;
  createdBy: string;
}

const Course_levelSettings: React.FC = () => {
  // Modal
  ...
  // Table
  const [course_levels, setCourse_levels] = useState<Course_level[]>([]);
  ...
  // Fetch Course_levels
  const fetchCourse_levels = async () => {
    const res = await axios.get(`/api/settings/course_level`);
    if (res.data.ok) setCourse_levels(res.data.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/course_level/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/course_level`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/course_level/${id}`);
    fetchCourse_levels();
  };

  // Filter + Pagination
  const filtered = course_levels.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Course levels</h5>
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <DialogPanel className="bg-white dark:bg-[#0c1427] p-6 rounded-md w-full max-w-md">
            <h3 className="text-lg font-medium mb-4">
              {editId ? "Edit Course level" : "Add Course level"}
            </h3>
...
export default Course_levelSettings;
