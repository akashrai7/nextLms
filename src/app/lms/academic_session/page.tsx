...
interface Academic_session {
  _id: string;
  name: string;
  createdBy: string;
}

const Academic_sessionSettings: React.FC = () => {
  // Modal
  ...
  // Table
  const [academic_sessions, setAcademic_sessions] = useState<Academic_session[]>([]);
  ...
  const fetchAcademic_sessions = async () => {
    const res = await axios.get(`/api/settings/academic_session`);
    if (res.data.ok) setAcademic_sessions(res.data.data);
  };

  useEffect(() => {
    fetchAcademic_sessions();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/academic_session/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/academic_session`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/academic_session/${id}`);
    fetchAcademic_sessions();
  };

  // Filter + Pagination
  const filtered = academic_sessions.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Academic Session</h5>
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <DialogPanel className="bg-white dark:bg-[#0c1427] p-6 rounded-md w-full max-w-md">
            <h3 className="text-lg font-medium mb-4">
              {editId ? "Edit Academic Session" : "Add Academic Session"}
            </h3>
...
export default Academic_sessionSettings;
