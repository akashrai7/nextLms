...
interface Affiliation_board {
  _id: string;
  name: string;
  createdBy: string;
}

const Affiliation_boardSettings: React.FC = () => {
  // Modal
  ...
  // Table
  const [affiliation_boards, setAffiliation_boards] = useState<Affiliation_board[]>([]);
  ...
  // Fetch Affiliation_boards
  const fetchAffiliation_boards = async () => {
    const res = await axios.get(`/api/settings/affiliation_board`);
    if (res.data.ok) setAffiliation_boards(res.data.data);
  };

  useEffect(() => {
    fetchAffiliation_boards();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/affiliation_board/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/affiliation_board`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/affiliation_board/${id}`);
    fetchAffiliation_boards();
  };

  // Filter + Pagination
  const filtered = affiliation_boards.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Affiliation boards</h5>
      ...
      <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
        <div className="trezo-card-header mb-[20px] md:mb-[25px] sm:flex items-center justify-between">
          ...
          <form className="relative sm:w-[265px]">
            ...
            <input
              type="text"
              placeholder="Search afiliation board..."
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <DialogPanel className="bg-white dark:bg-[#0c1427] p-6 rounded-md w-full max-w-md">
            <h3 className="text-lg font-medium mb-4">
              {editId ? "Edit Affiliation board" : "Add Affiliation board"}
            </h3>
...
export default Affiliation_boardSettings;
