...
const Current_classSettings: React.FC = () => {
  // Modal
  ...
  // Table
  const [current_classs, setCurrent_classs] = useState<Current_class[]>([]);
  ...
  // Fetch Current_classs
  const fetchCurrent_classs = async () => {
    const res = await axios.get(`/api/settings/current_class`);
    if (res.data.ok) setCurrent_classs(res.data.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/current_class/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/current_class`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/current_class/${id}`);
    fetchCurrent_classs();
  };

  // Filter + Pagination
  const filtered = current_classs.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Current Classs</h5>
...
export default Current_classSettings;
