...
// // skipped/error rows from server
...
//       try {
//         const [clRes, streamRes, sectionRes, natRes, academicSessionsRes] = await Promise.all([
...
//         setAcademicSessions(academicSessionsRes?.data ?? []);
...
//         if (data.success === false) setMessage(data.message || "Upload failed.");
//         else if (data.summary) {
//           setMessage(`Import finished. Created: ${data.summary.created || 0}, Updated: ${data.summary.updated || 0}, Skipped: ${data.summary.skipped || 0}, Failed: ${data.summary.failed || 0}`);
//         } else if (typeof data.successCount !== "undefined") {
//           setMessage(`Import finished. ${data.successCount} records created. Errors: ${data.errorRows?.length ?? 0}`);
//         } else setMessage("Import completed.");
...
//       <h1 className="text-2xl font-semibold mb-4">Bulk Student Upload</h1>

//       {message && <div className="mb-4 p-3 bg-yellow-100 border border-yellow-300 rounded">{message}</div>}
...
//           <input type="text" className="w-full border p-2 rounded bg-gray-50" value="student" readOnly />
//         </div>
//       </div>

//       <div className="mb-4">
//         <label className="block mb-1 text-sm">Upload CSV / XLSX</label>
...
//       <div className="mb-4">
//         <label className="block mb-1 text-sm">Duplicate Handling</label>
//         <select
...
//           >
//             Download Error Report ({invalidCount})
//           </button>
//         )}

//        <button onClick={() => downloadRows(errorRows, "errors.xlsx")}>Download Errors</button>
//        <button onClick={() => downloadRows(skippedRows, "skipped.xlsx")}>Download Skipped</button>
...
//         {parsedRows.length > 0 && (
//           <div>
//             <h2 className="font-semibold mb-2">Parsed Preview</h2>
...
//               <tbody>
//                 {parsedRows.map(r => (
//                   <tr key={r.__rowNum__} className={r.valid ? "" : "bg-red-100"}>
//                     <td className="border px-2 py-1">{r.__rowNum__}</td>
//                     <td className="border px-2 py-1">{r.firstName}</td>
//                     <td className="border px-2 py-1">{r.lastName}</td>
//                     <td className="border px-2 py-1">{r.email}</td>
//                     <td className="border px-2 py-1">{r.mobile}</td>
//                     <td className="border px-2 py-1">{r.dob?.slice(0, 10) ?? ""}</td>
//                     <td className="border px-2 py-1">{r.valid ? "Valid" : r.errors.join("; ")}</td>
//                   </tr>
//                 ))}
//               </tbody>
//             </table>
//             <div className="mt-2 text-sm text-gray-700">
//               Valid rows: {validCount}, Invalid rows: {invalidCount}
//             </div>
//           </div>
//         )}
//       </div>
