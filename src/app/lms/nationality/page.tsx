...
const NationalitySettings: React.FC = () => {
  // Modal
  ...
  const fetchNationalities = async () => {
    const res = await axios.get(`/api/settings/nationality`);
    if (res.data.ok) setNationalities(res.data.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/nationality/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/nationality`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/nationality/${id}`);
    fetchNationalities();
  };

  // Filter + Pagination
  const filtered = nationalities.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Nationalities</h5>
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <DialogPanel className="bg-white dark:bg-[#0c1427] p-6 rounded-md w-full max-w-md">
            <h3 className="text-lg font-medium mb-4">
              {editId ? "Edit Nationality" : "Add Nationality"}
            </h3>
