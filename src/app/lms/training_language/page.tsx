...
interface Training_language {
  _id: string;
  name: string;
  createdBy: string;
}

const Training_languageSettings: React.FC = () => {
  // Modal
  ...
  // Table
  const [training_languages, setTraining_languages] = useState<Training_language[]>([]);
  ...
  // Fetch Training_languages
  const fetchTraining_languages = async () => {
    const res = await axios.get(`/api/settings/training_language`);
    if (res.data.ok) setTraining_languages(res.data.data);
  };

  useEffect(() => {
    fetchTraining_languages();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/training_language/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/training_language`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/training_language/${id}`);
    fetchTraining_languages();
  };

  // Filter + Pagination
  const filtered = training_languages.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Training_languages</h5>
...
export default Training_languageSettings;
