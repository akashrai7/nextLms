...
const Institute_typeSettings: React.FC = () => {
  // Modal
  ...
  // Table
  const [institute_types, setInstitute_types] = useState<Institute_type[]>([]);
  ...
  // Fetch Institute_types
  const fetchInstitute_types = async () => {
    const res = await axios.get(`/api/settings/institute_type`);
    if (res.data.ok) setInstitute_types(res.data.data);
  };

  useEffect(() => {
    fetchInstitute_types();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/institute_type/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/institute_type`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/institute_type/${id}`);
    fetchInstitute_types();
  };

  // Filter + Pagination
  const filtered = institute_types.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Institute types</h5>
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <DialogPanel className="bg-white dark:bg-[#0c1427] p-6 rounded-md w-full max-w-md">
            <h3 className="text-lg font-medium mb-4">
              {editId ? "Edit Institute type" : "Add Institute type"}
            </h3>
...
export default Institute_typeSettings;
