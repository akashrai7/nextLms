...
const GenderSettings: React.FC = () => {
  ...
  const fetchGenders = async () => {
    const res = await axios.get(`/api/settings/gender`);
    if (res.data.ok) setGenders(res.data.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/gender/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/gender`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/gender/${id}`);
    fetchGenders();
  };
