...
const Video_sourceSettings: React.FC = () => {
  // Modal
  ...
  // Table
  const [video_sources, setVideo_sources] = useState<Video_source[]>([]);
  ...
  // Fetch video_sources
  const fetchVideo_sources = async () => {
    const res = await axios.get(`/api/settings/video_source`);
    if (res.data.ok) setVideo_sources(res.data.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await axios.put(`/api/settings/video_source/${editId}`, form);
      setEditId(null);
    } else {
      await axios.post(`/api/settings/video_source`, {
        ...form,
  ...
  const handleDelete = async (id: string) => {
    await axios.delete(`/api/settings/video_source/${id}`);
    fetchVideo_sources();
  };

  // Filter + Pagination
  const filtered = video_sources.filter((g) =>
  ...
  return (
    <>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">Video sources</h5>
      ...
      <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
        <div className="trezo-card-header mb-[20px] md:mb-[25px] sm:flex items-center justify-between">
          ...
          <form className="relative sm:w-[265px]">
            ...
            <input
              type="text"
              placeholder="Search Video source..."
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <DialogPanel className="bg-white dark:bg-[#0c1427] p-6 rounded-md w-full max-w-md">
            <h3 className="text-lg font-medium mb-4">
              {editId ? "Edit Video source" : "Add Video source"}
            </h3>
...
export default Video_sourceSettings;
